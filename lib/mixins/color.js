...
export default {
  ...
  _setColor(color, stroke) {
    if (color instanceof PDFGradient) {
      ...
      this.addContent(`/CS${this.spotColors[color].index} ${stroke ? 'CS' : 'cs'} 1 ${stroke ? 'SCN' : 'scn'}`);
      return false;
  ...
  addSpotColor(name, c, m, y, k) {
    ...
    const data = `[/Separation /${name.replace(/\s/g, '#20')} /DeviceCMYK << /Range [0 1 0 1 0 1 0 1] /C0 [0 0 0 0] /C1 [${
        values.map(value=>value/100.0).join(' ')
